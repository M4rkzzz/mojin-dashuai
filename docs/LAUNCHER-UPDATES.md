...
按用户要求保留版本号的 beta.10 测速修正版采用独立 latency2 发布目录和更高的清单序号 12，旧哈希对象保留。原 beta.10 按版本比较不会自动安装同版本修正，需覆盖安装；beta.7 等更早版本仍可通过自动更新获得该包。发布命令增加 `--revision latency2`，避免替换已有命名对象。

beta.11 使用新版本号及序号 13，beta.10 可在登录前自动升级。右上角最小化按钮左侧增加绿色“更新”，登录页和大厅共用状态。已确认新版的下载或启动失败保留版本信息，以便从该入口重试；较晚的初始状态响应不能覆盖实时进度。公开签名、完整 ZIP 回读、实际文件差异下载、原生辅助进程握手与静默安装记录见 `packs/beta11-acceptance.json`。
