...
启动器从 `https://launcher.boshan.uk/v1/launcher` 读取 ECDSA P-256 签名清单。未开放时返回 404；当前已提供 0.1.2-beta.7。ZIP 通过独立下载源读取，不附带账号凭据。
...
- API 0.1.4 已部署；0.1.2-beta.7 ZIP 已上传，beta 更新接口已激活。哈希及下载结果见 `packs/launcher-update-acceptance.json`。
