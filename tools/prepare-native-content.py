...
for instance in args.instances:
    ...
    if instance in ('m3e', 'mb'):
        adapter = ROOT / 'artifacts/game-integration' / ('mojin-autoconnect-1.7.10-0.1.0.jar' if instance == 'm3e' else 'mojin-autoconnect-cleanroom-0.1.0.jar')
        ...
        records[relative] = record(adapter, relative, [object_source(adapter, hashes(adapter)[1])],
                                   basis='Launcher-owned client connection adapter; source in src/GameIntegration/' + instance)
