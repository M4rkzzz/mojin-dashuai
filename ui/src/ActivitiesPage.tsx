...
const icons=itemIcons as Record<string,{src:string;name:string;block:boolean;overlay:string|null}>;
function RewardItems({world,items}:{world:string;items:Item[]}){
 return <div className="activity-items">{items.map((item,index)=>{const icon=icons[`${world}:${item.id}@${item.meta}`];return <div className="activity-item" key={`${item.id}-${item.meta}-${index}`}>
  {icon&&<span className={`item-texture ${icon.block?'item-block':''}`} aria-hidden="true">{icon.block?<span className="item-cube">{['top','left','right'].map(face=><span key={face} className={`cube-${face}`} style={{backgroundImage:`url('${icon.src}')`}}>{icon.overlay&&<img src={icon.overlay} alt=""/>}</span>)}</span>:<img src={icon.src} alt=""/>}</span>}
 ...
 return <section className="activities" style={{'--activity-accent':world.color} as React.CSSProperties}>
  ...
  {!data?<p role="status">{error?'暂时无法读取活动，已获得的进度和奖励会保留。':'正在读取活动进度…'}</p>:<>
   ...
   {tab==='draw'&&<><div className={`activity-draw ${drawing?'is-drawing':''}`}><img className="supply-chest" src="./activities/supply-chest.png" alt="" aria-hidden="true"/><div className="draw-copy"><h2>本服补给抽奖</h2><p>普通 80% · 精选 18% · 稀有 2%（基础概率）</p><p>最多再抽 <strong>{data.guaranteeWithin}</strong> 次获得稀有奖励。券与保底不会过期。</p></div><button className="primary" disabled={busy||data.tickets<1} onClick={()=>run('draw')}><Ticket size={18}/>{drawing?'正在开奖…':'使用 1 张抽奖券'}</button></div>
    ...
    <details className="activity-panel pool-details"><summary>查看奖池与领取条件</summary><p>稀有奖只补当前目标中已经掌握的材料；单次约 15%，同目标累计不超过 30%。</p>{data.pool.map(r=><div className="reward-pool-row" key={r.id}><strong>{r.name} · {rarity[r.tier]}</strong><span>{r.eligibility||(r.eligible?'已满足领取条件。':'满足原有任务与首次制作条件后开放。')}</span><RewardItems world={worldId} items={r.items}/><p>{r.purpose}</p></div>)}</details>
    <h2 className="activity-subtitle">奖励记录</h2><div className="award-list">{data.awards.length===0?<p>完成行动并领取签到，即可获得第一张抽奖券。</p>:data.awards.map(a=><article key={a.id} className={`activity-panel award-${a.tier}`}><div className="award-heading"><strong>{a.name||rarity[a.tier]}</strong><span>{status[a.status]}</span></div><small>{a.source} · {new Date(a.createdAt).toLocaleDateString('zh-CN')}</small><RewardItems world={worldId} items={a.items}/>{a.status==='queued'&&<p>入服后自动送入背包；背包满时继续保留。</p>}{a.status==='pending'&&(a.choices.length?<div className="rare-choices">{a.choices.map(r=><div key={r.id}><strong>{r.name}</strong><RewardItems world={worldId} items={r.items}/><p>{r.purpose}</p><button className="secondary" disabled={busy} onClick={()=>run('select',{awardId:a.id,rewardId:r.id})}>选择这份材料</button></div>)}</div>:<p>尚无符合当前制作条件的材料包，这份奖励会继续保留。</p>)}</article>)}</div>
   </>}
