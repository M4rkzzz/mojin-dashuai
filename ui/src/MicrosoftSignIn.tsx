...
export function MicrosoftSignIn({code,copy,open,cancel,expired}:{code:MicrosoftCode|null;copy:()=>Promise<void>;open:()=>Promise<void>;cancel:()=>Promise<void>;expired:()=>void}){
 ...
 return <section className="login-panel microsoft-login" aria-labelledby="microsoft-heading">
  ...
  </>:<div className="device-status" role="status"><RefreshCw size={18} className="spin"/>{cancelling?'正在取消':'正在获取登录码'}</div>}
