...
try{
 ...
 for(const width of [1280,820]){
  ...
  await expect(notice).toHaveText('更新');
  ...
  expect(Math.abs(button.x+button.width-minimize.x)).toBeLessThan(1);
  await notice.click();
  ...
  if(width===1280)await page.screenshot({path:'../.local/beta11-update-login.png',animations:'disabled'});
  ...
  await page.evaluate(()=>window.emitUpdate({phase:'current'}));await expect(notice).toHaveCount(0);
 }
 ...
 fs.writeFileSync('../.local/launcher-update-ui-check.json',JSON.stringify({passed:true,progress:true,restartGuard:true,retryAfterError:true,greenTitlebarNotice:true,beforeLogin:true,compactLayout:true,staleSnapshotIgnored:true,errors},null,2));
